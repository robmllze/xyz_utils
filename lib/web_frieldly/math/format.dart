...
/// 5.123.
double rountToFigures(double x, int figures) {
  assert(figures >= 0);
  if (x == 0) return 0;
  final a = log10(x).truncateToDouble();
  final y = pow(10, a + 1 - figures);
  return y * (x / y).roundToDouble();
}

double truncToFigures(double x, int figures) {
  assert(figures >= 0);
  if (x == 0) return 0;
  final a = log10(x).truncateToDouble();
  final y = pow(10, a + 1 - figures);
...
/// 5.1235.
double roundAt(double x, int figures) {
  assert(figures >= 0);
  if (x == 0) return 0;
  final y = pow(10, figures).toDouble();
  return (x * y).roundToDouble() / y;
}

double truncateAt(double x, int figures) {
  assert(figures >= 0);
  if (x == 0) return 0;
  final y = pow(10, figures).toDouble();
