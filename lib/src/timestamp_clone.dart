...
class TimestampClone implements Comparable<TimestampClone> {
  /// Creates a [TimestampClone]
  TimestampClone(this._seconds, this._nanoseconds) {
    _validateRange(_seconds, _nanoseconds);
  }

  /// Create a [TimestampClone] fromMillisecondsSinceEpoch
  factory TimestampClone.fromMillisecondsSinceEpoch(int milliseconds) {
    int seconds = (milliseconds / _kThousand).floor();
    final int nanoseconds = (milliseconds - seconds * _kThousand) * _kMillion;
    return TimestampClone(seconds, nanoseconds);
  }

  /// Create a [TimestampClone] fromMicrosecondsSinceEpoch
  factory TimestampClone.fromMicrosecondsSinceEpoch(int microseconds) {
    final int seconds = microseconds ~/ _kMillion;
    final int nanoseconds = (microseconds - seconds * _kMillion) * _kThousand;
    return TimestampClone(seconds, nanoseconds);
  }

  /// Create a [TimestampClone] from [DateTime] instance
  factory TimestampClone.fromDate(DateTime date) {
    return TimestampClone.fromMicrosecondsSinceEpoch(date.microsecondsSinceEpoch);
  }

  /// Create a [TimestampClone] from [DateTime].now()
  factory TimestampClone.now() {
    return TimestampClone.fromMicrosecondsSinceEpoch(
  ...
  /// Converts [TimestampClone] to [DateTime]
  DateTime toDate() {
  ...
  bool operator ==(Object other) =>
      other is TimestampClone && other.seconds == seconds && other.nanoseconds == nanoseconds;

  @override
  int compareTo(TimestampClone other) {
